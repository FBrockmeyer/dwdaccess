...
> The output from the *code chunk* above cannot be presented on github, since support for *java*-driven applications like *leaflet* > is missing. Come back later to see an update.  
...
```{r, include=FALSE, fig.cap = "How the author escaped the heat in 1994.", fig.align = "center", out.width="300px"}
...
For the given span of years, the total amount of days per year falling into each day category can be visualised: 
...
>Note, years with missing data are cause white space inside the figures. 
...
Build a plot with two y-axis: on the left *monthly amount of rainfall*, and on the right *average monthly temperature*.
...
month_lables <- c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 
